...
lint: clean
	poetry run flake8 .
	poetry run pylint --disable=all --enable=E0602,E0603,E1101 stackinsights tests
...
.PHONY: license
license: clean
	docker run -it --rm -v $(shell pwd):/github/workspace ghcr.io/apache/stackinsights-eyes/license-eye:20da317d1ad158e79e24355fdc28f53370e94c8a header check

.PHONY: test
test: env
	sudo apt-get -y install jq
	docker build --build-arg BASE_PYTHON_IMAGE=3.7-slim -t apache/stackinsights-python-agent:latest-plugin --no-cache . -f tests/plugin/Dockerfile.plugin
...
clean:
	rm -rf stackinsights/protocol
	rm -rf apache_stackinsights.egg-info dist build
	rm -rf stackinsights-python*.tgz*
...
release: clean lint license
	tar -zcvf stackinsights-python-src-$VERSION.tgz --exclude .venv *
	gpg --batch --yes --armor --detach-sig stackinsights-python-src-$VERSION.tgz
	shasum -a 512 stackinsights-python-src-$VERSION.tgz > stackinsights-python-src-$VERSION.tgz.sha512
