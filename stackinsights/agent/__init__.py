...
from stackinsights import config, plugins
from stackinsights import loggings
from stackinsights import meter
from stackinsights import profile
from stackinsights.agent.protocol import Protocol, ProtocolAsync
from stackinsights.command import command_service, command_service_async
from stackinsights.loggings import logger
from stackinsights.profile.profile_task import ProfileTask
from stackinsights.profile.snapshot import TracingThreadSnapshot
from stackinsights.protocol.language_agent.Meter_pb2 import MeterData
from stackinsights.protocol.logging.Logging_pb2 import LogData
from stackinsights.utils.singleton import Singleton

if TYPE_CHECKING:
    from stackinsights.trace.context import Segment
...
class StackInsightsAgent(Singleton):
    """
    The main singleton class and entrypoint of StackInsights Python Agent.
    ...
    def __bootstrap(self):
        ...
        if config.agent_protocol == 'grpc':
            from stackinsights.agent.protocol.grpc import GrpcProtocol
            self.__protocol = GrpcProtocol()
        elif config.agent_protocol == 'http':
            from stackinsights.agent.protocol.http import HttpProtocol
            self.__protocol = HttpProtocol()
        elif config.agent_protocol == 'kafka':
            from stackinsights.agent.protocol.kafka import KafkaProtocol
    ...
    def __init_threading(self) -> None:
        ...
        if config.agent_meter_reporter_active:
            ...
            if config.agent_pvm_meter_reporter_active:
                from stackinsights.meter.pvm.cpu_usage import CPUUsageDataSource
                from stackinsights.meter.pvm.gc_data import GCDataSource
                from stackinsights.meter.pvm.mem_usage import MEMUsageDataSource
                from stackinsights.meter.pvm.thread_data import ThreadDataSource
    ...
    def __fork_before() -> None:
        ...
        logger.warning('StackInsights Python agent fork support is currently experimental, '
    ...
    def __fork_after_in_child(self) -> None:
        ...
        logger.info('New process detected, re-initializing StackInsights Python agent')
    ...
    def start(self) -> None:
        ...
        if sys.version_info < (3, 7):
            ...
            logger.warning('StackInsights Python agent does not support Python 3.6 and below, '
        ...
        if not self.__started:
            ...
            self.__started = True
            logger.info(f'StackInsights sync agent instance {config.agent_instance_name} starting in pid-{os.getpid()}.')

            # Install log reporter core
            # TODO - Add support for printing traceID/ context in logs
            if config.agent_log_reporter_active:
                from stackinsights import log
                log.install()
            ...
            raise RuntimeError('StackInsights Python agent has already been started in this process, '
...
class StackInsightsAgentAsync(Singleton):
    __started: bool = False  # shared by all instances
    ...
    def __bootstrap(self):
        if config.agent_protocol == 'grpc':
            from stackinsights.agent.protocol.grpc_aio import GrpcProtocolAsync
            self.__protocol = GrpcProtocolAsync()
        elif config.agent_protocol == 'http':
            from stackinsights.agent.protocol.http_aio import HttpProtocolAsync
            self.__protocol = HttpProtocolAsync()
        elif config.agent_protocol == 'kafka':
            from stackinsights.agent.protocol.kafka_aio import KafkaProtocolAsync
    ...
    def start(self) -> None:
        ...
        if not self.__started:
            ...
            self.__started = True
            logger.info(f'StackInsights async agent instance {config.agent_instance_name} starting in pid-{os.getpid()}.')
...
# so users still use `from stackinsights import agent`
agent = StackInsightsAgentAsync() if config.agent_asyncio_enhancement else StackInsightsAgent()
start = agent.start
