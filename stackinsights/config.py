...
kafka_topic_management: str = os.getenv('SW_KAFKA_TOPIC_MANAGEMENT', 'stackinsights-managements')
# Specifying Kafka topic name for Tracing data, this should be in sync with OAP
kafka_topic_segment: str = os.getenv('SW_KAFKA_TOPIC_SEGMENT', 'stackinsights-segments')
# Specifying Kafka topic name for Log data, this should be in sync with OAP
kafka_topic_log: str = os.getenv('SW_KAFKA_TOPIC_LOG', 'stackinsights-logs')
# Specifying Kafka topic name for Meter data, this should be in sync with OAP
kafka_topic_meter: str = os.getenv('SW_KAFKA_TOPIC_METER', 'stackinsights-meters')
...
# Use TLS for communication with StackInsights OAP (no cert required)
...
# backend, refer to [the yaml](https://github.com/apache/stackinsights/blob/4f0f39ffccdc9b41049903cc540b8904f7c9728e/
...
# The agent will exchange heartbeat message with StackInsights OAP backend every `period` seconds
...
# short-lived processes as each one will create a new instance in StackInsights dashboard
...
# `sw-python` CLI, if set to `False`, a valid child process will not boot up a StackInsights Agent. Please refer to the
...
def init(**kwargs) -> None:
    """
    Used to initialize the configuration of the StackInsights Python Agent.
    Refer to the official online documentation
    https://stackinsights.apache.org/docs/stackinsights-python/next/en/setup/configuration/
...
def finalize_name() -> None:
    ...
    Ref https://github.com/apache/stackinsights-java/pull/123
    """
    global agent_name
    if agent_namespace:
