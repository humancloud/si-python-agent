...
1. initialize the StackInsights Python Agent.
...
def _get_sw_loader_logger():
    """ Setup a new logger with passed stackinsights CLI env vars,
    don't import from stackinsights, it may not be on sys.path
    if user misuses the CLI to run programs out of scope
    """
    from logging import getLogger
    logger = getLogger('stackinsights-loader')
...
if sys.prefix != sys.base_prefix:
    _sw_loader_logger.debug('[The StackInsights agent bootstrapper is running inside a virtual environment]')
...
# their Python env doesn't have StackInsights installed. Or even call another
# env that has another version of StackInsights installed, which is leads to problems.
# Even if `import stackinsights` was successful, doesn't mean its the same one where sw-python CLI was run.
...
if not (version_match and prefix_match):
    ...
    _sw_loader_logger.error('The sw-python CLI was instructed to run a program '
                            ...
                            'Please make sure that sw-python CLI, stackinsights agent and your '
    ...
    # also override debug for stackinsights agent itself
    if os.environ.get('SW_AGENT_SW_PYTHON_CLI_DEBUG_ENABLED') == 'True':  # set from the original CLI runner
    ...
    try:
        _sw_loader_logger.debug('StackInsights Python Agent starting, loader finished.')
        ...
        _sw_loader_logger.exception('StackInsights Python Agent failed to start, please inspect your package installation.'
