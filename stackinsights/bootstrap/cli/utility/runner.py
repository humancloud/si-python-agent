...
from stackinsights.bootstrap import cli_logger
from stackinsights.bootstrap.cli import SWRunnerFailure
...
def prefork_handler(command: List[str]) -> None:
    ...
    if command[0] == 'gunicorn':
        ...
        cli_logger.info('We noticed you are using Gunicorn, '
                        'agent will automatically start the StackInsights Python Agent'
        ...
        cli_logger.info('We noticed you are using uWSGI, '
                        ...
                        'We will also start the StackInsights Python Agent in all child (worker) '
...
def execute(command: List[str], experimental_check_prefork: bool) -> None:
    ...
    cli_logger.debug(f'StackInsights Python agent `runner` received command {command}')
    ...
    from stackinsights.bootstrap.loader import __file__ as loader_dir
    from stackinsights.bootstrap.hooks import __file__ as hook_dir
