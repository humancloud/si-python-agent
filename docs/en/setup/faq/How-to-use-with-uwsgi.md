...
* [[Python] Apache Stackinsights, flask uwsgi, no metrics send to server · Issue #6324 · apache/stackinsights](https://github.com/apache/stackinsights/issues/6324)
* [[Bug] stackinsights-python not work with uwsgi + flask in master workers mode and threads mode · Issue #8566 · apache/stackinsights](https://github.com/apache/stackinsights/issues/8566)
...
If you get some problems when using StackInsights Python agent, you can try to use the following manual method to call [`@postfork`](https://uwsgi-docs.readthedocs.io/en/latest/PythonDecorators.html#uwsgidecorators.postfork), the low-level API of uWSGI to initialize the agent.
...
@postfork
def init_tracing():
    # Important: The import of stackinsights must be inside the postfork function
