...
These blogs [stackinsights-profiling](https://stackinsights.apache.org/blog/2020-04-13-apache-stackinsights-profiling/#profiling-in-production) and [stackinsights-python-profiling](https://stackinsights.apache.org/blog/2021-09-12-stackinsights-python-profiling/) described how the threading-profiler works
...
In this circumstance, the snapshot of the working thread includes multi contexts of different greenlets, which will make stackinsights confused to build the trace stack.
...
We can set a callback function to the greenlet that we need to profiling, then when the greenlet.HUB switches the context in/out to the working thread, the callback will build a snapshot of the greenlet's traceback and send it to stackinsights.
